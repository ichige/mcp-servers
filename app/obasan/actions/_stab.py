...
async def do_information():
    """
    ファイル情報を返すスタブ
    """

    await markdown.render_stream('''
# Information
...
| 更新対象可否 | False |
| 最終更新日時 | 2026/04/01 |
| ファイルサイズ(bytes) | 12.4MB)
| 最終コミット日時 | 2026/03/13 |
    ''')

    phase.update("information")
    await chat_messages.reply_message_stream("お待たせしますたっす")
...
```
    ...
    await chat_messages.reply_message_stream("お待たせいたしました。")
    phase.update("translated")