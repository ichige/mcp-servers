...
`@step` デコレータでイベント駆動による状態遷移を実現させる仕組みだが、この設計がイマイチに感じる場合がある。  

まずこの `@step` は定義している Workflow クラスの中でしか定義できない。  
※ Free function step という方法もあるが、同じファイル内で定義する必要がある。

おそらくは関数の引数の型と戻り値の型を静的に解析し、イベントと関数呼び出しのマッピングを動的に実現していると思われるが、
それゆえ同じクラス(ファイル)内で定義する必要があり、Mixin や外部ファイルでの定義が出来ないようになっている。  

これは `@step` 関数の再利用が出来ないことを示し、同時に Workflow を肥大化させる要因にもなる。  
※ Mixin(多重継承) とかでもダメなんですよー。

見通しが悪くなったり、ロジックの再利用が必要な場合は、処理を関数化して外で定義して `@step` 内で呼び出す(少々スマートになる？)しかなさそうである。  
魔法を使う代償とはこういう事なんでしょうな。

### UNIX TIMESTAMP 変換しない？

数字には弱い？と聞いてはいたが、UNIX TIMESTAMP の変換も出来ないようであった。  
それっぽい日付を表示していたが、まさかのでっちあげとは！

ということで変換ツールを用意したら、それを使うようにはなった。  
そもそもファイルのメタ情報を返すツールでで変換してても良かったけど、あえて LLM にやらせようという試みだったが失敗。  
しかし変換ツールを実行するのである意味成功とも言える。